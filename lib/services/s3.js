...
AWS.util.update(AWS.S3.prototype, {
  ...
  shouldDisableBodySigning: function shouldDisableBodySigning(request) {
    ...
    if (this.config.s3DisableBodySigning === true
      && signerClass === AWS.Signers.V4) {
      return true;
    }
    return false;
  },
  ...
  removeVirtualHostedBucketFromPath: function removeVirtualHostedBucketFromPath(req) {
    ...
    if (bucket && httpRequest.path) {
      ...
      if (httpRequest.path.startsWith('/' + bucket)) {
        httpRequest.path = httpRequest.path.substr(1 + bucket.length);
      }
  ...
  addExpect100Continue: function addExpect100Continue(req) {
    ...
    if (AWS.util.isNode() && false && (len >= 1024 * 1024 || req.params.Body instanceof AWS.util.stream.Stream)) {
  ...
  isDnsCompatible: function isDnsCompatible(bucketName) {
    if (dnsCompatibleCache[bucketName] != null) {
      return dnsCompatibleCache[bucketName];
    }
    var b = bucketName;
    ...
    var dots = new RegExp(/\.\./);
    return dnsCompatibleCache[bucketName] =
      (b.match(domain) && !b.match(ipAddress) && !b.match(dots)) ? true : false;
  },
